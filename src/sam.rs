pub mod sam {
    ...
    impl SAM {
        ...
        pub fn create_session(self: &mut Self, raw_nickname: &str) -> bool {
            ...
            let re = Regex::new(r"SESSION STATUS RESULT=(\w+) DESTINATION=.+")
                .expect("cant compile regex");
            dbg!(&line);
        ...
        pub fn connect(self: &mut Self, destination: &str) -> bool {
            ...
            self.write(format!(
                ...
                self.nickname, destination
            ))
            .expect("Can't connect to destination");
            let mut buffer = [0u8; 2056];
            let _ = self.t_stream.read(&mut buffer);
            let line = String::from_utf8_lossy(&buffer);
            println!("{}", line);
        ...
        pub fn generate_dest(self: &mut Self, v: u8) -> KeyPair {
            self.write(format!("DEST GENERATE SIGNATURE_TYPE={}\n", v))
                .expect("Can't generate address [write]");
            let mut buffer = [0u8; 2056];
            let _ = self.t_stream.read(&mut buffer);
            let line = String::from_utf8_lossy(&buffer);
            //line.to_string()

            let re = Regex::new(r"DEST REPLY PUB=([\w\-~=]+) PRIV=([\w\-~=]+)")
        ...
        pub fn new(host: &str, port: u16) -> Self {
            ...
            let mut stream = match std::net::TcpStream::connect(h) {
                Ok(x) => x,
                Err(_) => {
            ...
            if stream
                .write(HANDSHAKE_MESSAGE.as_bytes())
